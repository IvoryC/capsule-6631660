...
# def simulate_plate(colors = ['#3B46B0', 'red'], 
#                   color_negs=True):
...
#     cmap_samp=matplotlib.colors.LinearSegmentedColormap.from_list("", colors+['#3B46B0']+['#3B46B0'])
...
def simulate_plate(neg_format = pd.DataFrame({'V1': {0: 1, 1: 12, 2: 1, 3: 12}, 
                                              'V2': {0: 1, 1: 8, 2: 8, 3: 1}}), 
                   colors = ['blue', 'red'], 
                  color_negs=True):
    ...
    norm=plt.Normalize(-2,2)
    cmap = matplotlib.colors.LinearSegmentedColormap.from_list("",['#3B46B0']*2 + colors )
    cmap_samp=matplotlib.colors.LinearSegmentedColormap.from_list("", colors+['#3B46B0']*2)
    blank_map = matplotlib.colors.LinearSegmentedColormap.from_list("", ['white', 'white'])
    ...
    for a in range(1, N+1):
        for b in range(1, M+1):
            if sum([(q,w)==(a,b) for q,w in zip(neg_format.V2, neg_format.V1)])>=1:
                ta=100*b
                tb=100*a
                ...
                ax.add_patch(circ)
                ax.imshow(1-data, clip_path=circ, origin='lower', 
                          cmap=[cmap if color_negs else blank_map][0]
                         )
            else:
                circ = patches.Circle((100*b,100*a),radius=rad, transform=ax.transData, 
                                       linewidth=2, 
