...
def make_supplemental_jsd_swarmplot_data( datasets ):
    ...
    cnts = np.hstack([ datasets_high_prev[a].T.sum(axis=0)
                      for a in datasets_high_prev])
    ...
    cnts = np.hstack([ datasets_low_prev[a].T.sum(axis=0)
                      for a in datasets_low_prev])
...
def make_supplemental_jsd_swarmplot(datasets, path='../results/Supplementary_Figures/F_S7/F_JSD.pdf', 
                                   hide_axes=False):
    ...
    print(scipy.stats.wilcoxon(full_plot_df.loc[(full_plot_df.Dataset=='Decontam_LB')&(full_plot_df.Group=='Low-Prevalence')].Value.values, 
                               full_plot_df.loc[(full_plot_df.Dataset=='Raw')&(full_plot_df.Group=='Low-Prevalence')].Value.values, alternative='greater' ) )
    ...
    print('Microdecon vs Decontam')            
    print(scipy.stats.wilcoxon(full_plot_df.loc[(full_plot_df.Dataset=='Decontam')&(full_plot_df.Group!='Low-Prevalence')].Value.values, 
                               full_plot_df.loc[(full_plot_df.Dataset=='microDecon')&(full_plot_df.Group!='Low-Prevalence')].Value.values, alternative='less' ) )
    
    print('Microdecon vs Decontam (LB)')            
    print(scipy.stats.wilcoxon(full_plot_df.loc[(full_plot_df.Dataset=='Decontam_LB')&(full_plot_df.Group!='Low-Prevalence')].Value.values, 
                               full_plot_df.loc[(full_plot_df.Dataset=='microDecon')&(full_plot_df.Group!='Low-Prevalence')].Value.values, alternative='less' ) )

    print('Microdecon vs Raw')            
    print(scipy.stats.wilcoxon(full_plot_df.loc[(full_plot_df.Dataset=='Raw')&(full_plot_df.Group!='Low-Prevalence')].Value.values, 
                               full_plot_df.loc[(full_plot_df.Dataset=='microDecon')&(full_plot_df.Group!='Low-Prevalence')].Value.values, alternative='less' ) )
    
    print('Microdecon vs Restrictive')            
    print(scipy.stats.wilcoxon(full_plot_df.loc[(full_plot_df.Dataset=='Restrictive')&(full_plot_df.Group!='Low-Prevalence')].Value.values, 
                               full_plot_df.loc[(full_plot_df.Dataset=='microDecon')&(full_plot_df.Group!='Low-Prevalence')].Value.values, alternative='less' ) )


    print(full_plot_df.shape)

    print(full_plot_df.loc[full_plot_df.Dataset=='Raw'].shape)
...
def generate_s7b(hide_axes):
    v3_sims=pd.read_csv('../results/data/Fig_2/Sensitivity/SMP_variation.csv', index_col=0)
    plt.figure(figsize=(3,7) )
    ax=sns.swarmplot(x='name', 
                 y='composition', 
                  data=v3_sims.loc[v3_sims.name=='SCRuB'],
                palette=global_palette
                )
    plt.ylabel('Relative abundance of true strin\nin simulated sample')
    ...
    plt.savefig('../results/Supplementary_figures/F_S7/F_S7_B.pdf', 
               dpi=900, 
               format='pdf', 
               bbox_inches='tight')
