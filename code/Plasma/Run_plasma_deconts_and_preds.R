...
dir.create('../results/data/Fig4_plasma')
...
divers_df %>% write.csv('../results/data/Fig4_plasma/plasma_shannon_diversities.csv')
...
dir.create(paste0('../results/data/Fig4_plasma/trial_', seed) )
dir.create(paste0('../results/data/Fig4_plasma/trial_', seed, '/scrubbed_preds') )
dir.create(paste0('../results/data/Fig4_plasma/trial_', seed, '/raw_preds') )
dir.create(paste0('../results/data/Fig4_plasma/trial_', seed, '/dec_preds') )
...
for(idx in 1:ncol(xy)){
  raw_hVsC_tmp <- loocvDTs(snmData = dec_standard_normalized,
                           samplingSize = sampling_sizes[idx], 
                           DTs = xy[ ,idx],
                           filenameString = paste0('../results/data/Fig4_plasma/trial_', seed, '/dec_standard/', xy[,idx][1], '_', xy[,idx][2] ),
                           caretTuneGrid = defaultGBMGrid)
}
...
for(idx in 1:ncol(xy)){
  raw_hVsC_tmp <- loocvDTs(snmData = dec_lb_normalized, 
                           samplingSize = sampling_sizes[idx], 
                           DTs = xy[ ,idx],
                           filenameString = paste0('../results/data/Fig4_plasma/trial_', seed, '/dec_lb/', xy[,idx][1], '_', xy[,idx][2] ),
                           caretTuneGrid = defaultGBMGrid)
}
...
for(idx in 1:ncol(xy)){
  raw_hVsC_tmp <- loocvDTs(snmData = restrictive_normalized, 
                           samplingSize = sampling_sizes[idx], 
                           DTs = xy[ ,idx],
                           filenameString = paste0('../results/data/Fig4_plasma/trial_', seed, '/restrictive_preds/', xy[,idx][1], '_', xy[,idx][2] ),
                           caretTuneGrid = defaultGBMGrid)
}
